...
export const AdminTasksForm: React.FC = (): JSX.Element => {
    ...
    const createOrUpdateTask = async (values: TaskRequest, setSubmitting: (isSubmitting: boolean) => void) => {
        try {
            ...
            toast({
                title: `Tätigkeit konnte nicht ${isAddMode ? 'erstellt' : 'aktualisiert'} werden`,
                description: errMsg,
                status: 'error',
                duration: 4000,
                isClosable: true
    ...
    return (
        <Container maxW='container.md' bg='whiteAlpha.900' boxShadow='md' p={0}>
            <Heading size='md' bg='secondary.500' color='white' px={4} py={2}>Tätigkeit {isAddMode ? 'anlegen' : 'bearbeiten'}</Heading>
